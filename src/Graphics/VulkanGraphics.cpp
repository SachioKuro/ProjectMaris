...
std::vector<char> VulkanGraphics::readShaderFile(const std::string& filename) {
...
void VulkanGraphics::Initialize(IWindow* window){
    m_window = window;
    m_currentImageIndecies.resize(MAX_FRAMES_IN_FLIGHT);
...
void VulkanGraphics::BeginFrame() {
    ...
    VkResult result = vkAcquireNextImageKHR(
        ...
        VK_NULL_HANDLE,
        &m_currentImageIndecies[m_currentFrame]
    );
...
void VulkanGraphics::EndFrame() {
    ...
    presentInfo.pImageIndices = &m_currentImageIndecies[m_currentFrame];
...
void VulkanGraphics::Draw() {
    ...
    recordCommandBuffer(m_commandBuffers[m_currentFrame], m_currentImageIndecies[m_currentFrame]);
}
...
void VulkanGraphics::createInstance() {
    ...
    appInfo.pEngineName = "Project Maris";
...
bool VulkanGraphics::checkValidationLayerSupport() {
    ...
    return std::all_of(
        m_validationLayers.begin(),
        m_validationLayers.end(),
        [availableLayers](auto layer) { 
            return std::find_if(
                availableLayers.begin(),
                availableLayers.end(),
                [layer](auto avLayer) { return strcmp(layer, avLayer.layerName) == 0; }) != availableLayers.end();
        }
    );
}
...
void VulkanGraphics::populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo) {
    createInfo = {};
...
void VulkanGraphics::createSurface() {
    if (glfwCreateWindowSurface(m_instance, (GLFWwindow*)m_window->GetWindowObj(), nullptr, &m_surface) != VK_SUCCESS) {
...
bool VulkanGraphics::isDeviceSuitable(VkPhysicalDevice device) {
    ...
    isSuitable &= findQueueFamilies(device).isComplete();
...
VkExtent2D VulkanGraphics::chooseSwapExtent(const VkSurfaceCapabilitiesKHR capabilities) {
    if (capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max()) {
        ...
        int width, height;
        glfwGetFramebufferSize((GLFWwindow*)m_window->GetWindowObj(), &width, &height);
...
void VulkanGraphics::recreateSwapChain() {
    // refactor this to the actual window
